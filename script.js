  ...
  WordSearch.prototype.drawmatrix = function () {
    for (var row = 0; row < this.settings.gridSize; row++) {
      ...
      for (var col = 0; col < this.settings.gridSize; col++) {
        ...
        cvEl.setAttribute("width", 200);
        cvEl.setAttribute("height", 200);
        cvEl.setAttribute(
          "style",
          "background-image: url(./images/" +
            this.matrix[row][col].letter +
            ".jpg); background-size: auto;  background-size: cover;"
        );
        ...
        ctx.fillStyle = "rgba(255, 255, 255, 0)"; // Text color
...
for (i in words) {
  ...
  var liIg = document.createElement("img");
  var image = "./images/" + i + ".jpg";
  liIg.setAttribute("src", image);
